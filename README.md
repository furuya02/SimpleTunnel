...
The Packet Tunnel Provider extension point is used to implemnt the client side of a custom network tunneling protocol that encapsulates network data in the form of IP packets. 
The PacketTunnel target in the SimpleTunnel project produces a **sample Packet Tunnel Provider extension**.

Packet Tunnel Provider拡張ポイントは、（IPパケットでネットワークデータをカプセリング化する）カスタムネットワークトンネリングプロトコルのクライアントサイドを実装するために使用されます。
...
The App Proxy Provider extension point is used to implement the client side of a custom network proxy protocol that encapsulates network data in the form of flows of application network data. Both TCP or stream-based and UDP or datagram-based flows of data are supported. The AppProxy target in the SimpleTunnel project produces a **sample App Proxy Provider extension**.
 ...
 App Proxy Provider拡張ポイントは、（アプリケーションのネットワーク・データの流れの形でネットワークデータをカプセル化する）カスタムネットワークプロキシプロトコルのクライアントを実装するために使用されます。
 TCPもUDPもどちらもサポートします。
